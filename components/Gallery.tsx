...
export default function Gallery() {
  return (
    <section id="gallery" className="w-full bg-[#F8F8F8] py-24 md:py-32 flex flex-col justify-center items-center overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {ITEMS.map((item) => (
            <div 
              key={item.id} 
              className={`relative overflow-hidden rounded-3xl border border-luxury-silver bg-white group shadow-sm transition-all duration-500 hover:shadow-md ${item.layoutClass}`}
            >
              ...
              <div className="relative w-full h-full">
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 text-white translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 flex flex-col space-y-2 pointer-events-none z-10">
                  <span className="text-[9px] tracking-widest uppercase font-mono font-bold text-neutral-300">
                    {item.category}
                  </span>
                  <h3 className="text-xl font-display font-light tracking-wide">
                    {item.title}
                  </h3>
                  <p className="text-xs font-light text-neutral-200 max-w-md">
                    {item.description}
                  </p>
                </div>

                {/* Subtle static label (disappears on hover) */}
                <div className="absolute top-6 right-6 glassmorphism px-4 py-2 rounded-full text-[8px] tracking-wider text-black uppercase font-semibold group-hover:opacity-0 transition-opacity duration-300 select-none pointer-events-none z-10">
                  {item.category}
                </div>
              </div>
            </div>
